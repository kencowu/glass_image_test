...
class PhoneImageProcessor:
    ...
    def detect_phones(self, image: np.ndarray) -> List[np.ndarray]:
        ...
        min_area = width * height * 0.02  # Lowered to 2% of image area for more flexibility
        phone_contours = []
        ...
        for contour in contours:
            ...
            print(f"Contour: area={area:.2f}, aspect_ratio={aspect_ratio:.2f}, w={w}, h={h}")
            if area > min_area and 0.2 < aspect_ratio < 1.0:
                phone_contours.append(contour)
                print(f"Accepted as phone: aspect_ratio={aspect_ratio:.2f}, area={area:.2f}")
        
        # Fallback: if no contour passes, use the largest contour if available
        if not phone_contours and contours:
            largest = max(contours, key=cv2.contourArea)
            phone_contours = [largest]
            print("No contour passed filters, using largest contour as fallback.")
    ...
    def _process_phone_image(self, phone_img: np.ndarray) -> np.ndarray:
        ...
        # --- Removed image rotation functionality ---
        # The image will not be rotated based on angle
        ...
        if contours:
            ...
            x = max(0, x - padding)
            y = max(0, y - padding)
            w = min(phone_img.shape[1] - x, w + 2 * padding)
            h = min(phone_img.shape[0] - y, h + 2 * padding)
            
            # Crop the image
            cropped = phone_img[y:y+h, x:x+w]
...
def main():
    ...
    # # Set input directory to correct path for this workspace
    # input_dir = "glass_image_test-main/preprocessed_images"
